...
export class HttpAdapter extends Adapter implements IHttpAdapter {
  ...
  find<R extends IRecord>(resource: IResource<R, this>, request: IHttpRequest): Promise<IResponse> {
    return this.dataLayer.find(this._getRequest(resource, request).merge(<IHttpRequest> {isArray: true}));
  }
  ...
  protected _getRequest<R extends IRecord>(resource: IResource<R, this>, request: IHttpRequest): HttpRequest {
    return new HttpRequest({url: `${this.baseUrl}/${resource.identity}`}).merge(request);
  }
}
